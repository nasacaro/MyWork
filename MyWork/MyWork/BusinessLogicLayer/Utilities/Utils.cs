...
namespace MyWork.BusinessLogicLayer.Utilities
{
    ...
    {
        private static Regex rgx = new Regex("^(0|84){1}");
        public static string FormatNumber(string msisdn)
        {
            if (msisdn == null)
                return "";

            return formatFullISDN(msisdn);
        }
        public static string shortenISDN(string strISDN)
        {
            if (strISDN == null)
                return "";
            return rgx.Replace(strISDN, "");
        }
        public static string formatNormalISDN(string strISDN)
        {
            if (strISDN == null)
                return "";
            return "0" + shortenISDN(strISDN);
        }
        ...
        public static bool IsValidMsisdn(string msisdn)
        {
            ...
            if (!pattern.IsMatch(msisdn) || !isMobileSubs(msisdn))
            {
                return false;
            }
            msisdn = formatFullISDN(msisdn);
            return msisdn.StartsWith("8491") || msisdn.StartsWith("8494") || msisdn.StartsWith("8488")
                || msisdn.StartsWith("84123") || msisdn.StartsWith("84124") || msisdn.StartsWith("84125") || msisdn.StartsWith("84127") || msisdn.StartsWith("84129");
        }

        public static bool isMobileSubs(string phoneNumber)
        {
            if ((phoneNumber.Trim().Length > 11 && phoneNumber.Substring(0, 2).Equals("01"))
                || (phoneNumber.Trim().Length > 10 && (phoneNumber.Substring(0, 2).Equals("09") || phoneNumber.Substring(0, 2).Equals("08")))
                || phoneNumber == null || phoneNumber.Length < 3 || (phoneNumber.Substring(0, 2).Equals("84")
                && !phoneNumber.Substring(0, 3).Equals("849") && !phoneNumber.Substring(0, 3).Equals("848") && !phoneNumber.Substring(0, 3).Equals("841")))
            {
                return false;
            }
            return true;
